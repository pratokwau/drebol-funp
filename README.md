...
2. **Открываешь игру** — внутри заводишь товары: вручную (название, ключевые
   слова, закуп) или кнопкой **«Подтянуть лоты с FunPay»** — тогда список лотов
   раздела появляется прямо здесь, остаётся вписать закуп напротив нужного.
   Уже заведённые лоты помечаются.
3. **Комиссия FunPay** задаётся сверху (по умолчанию 3%) и сохраняется.
